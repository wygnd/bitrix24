...
export class BitrixWikiUseCase {
  ...
  public async sendNoticeWaitingPayment(
    fields: B24WikiPaymentsNoticeWaitingOptions,
  ...
  public async sendNoticeReceivePayment(
    ...
    try {
      ...
      const [, , amount, reason, inn] = message.split(' | ');
      const messageKeyboard: B24ImKeyboardOptions[] = [];
      let resultMessage = '';
      let clientDepartmentHistory = '';
      const isBudget = message.includes('Бюджет');
      const clearInn = this.bitrixService.clearNumber(inn);
      const clearAmount = this.bitrixService.clearNumber(amount);
      const chatId =
      ...
      // Если СБП:
      //  - Отправляем сообщение в чат о поступлении платежа
      //  - Лайкаем сообщение о платеже
      //  - Генерируем счет
      //  - Отправляем данные в Wiki
      // if (is_sbp && isBudget) {
      //   const chatId: string | null =
      //     user_role in B24_WIKI_PAYMENTS_ROLES_CHAT_IDS
      //       ? B24_WIKI_PAYMENTS_ROLES_CHAT_IDS[user_role]
      ...
      //     ];
      //
      //     commands['notify_about_receive_payment'] = {
      //       method: 'imbot.message.add',
      //       params: {
      ...
      //       request: fields,
      //       error: `Не удалось найти чат по user_role: ${user_role}`,
      //     });
      //   }
      //
      //   if (
      ...
      //         // Отправляем данные в Wiki
      //
      //         await this.wikiService.sendReceivedPaymentData({
      //           amount: clearAmount,
      //           payment_type: 'spb',
      ...
      //           handler: this.sendNoticeReceivePayment.name,
      //           request: fields,
      //           error: 'Не удалось получить номер счета',
      //         });
      //       }
      //
      //       message = 'Авто генерация счета[br]';
      //     } catch (error) {
      //       this.logger.error({
      //         handler: this.sendNoticeReceivePayment.name,
      ...
      //       });
      //       message =
      //         'Ошибка авто генерации счета. Необходимо вручную выставить счет';
      //     }
      //
      //     commands['notify_manager'] = {
