...
export class BitrixBotUseCase {
  ...
  constructor(
    ...
    private readonly yandexDirectService: YandexDirectService,
  ) {}
  ...
  async handleApproveSiteDeal(
    ...
    try {
      ...
      let changeMessage =
        '[b]' +
        (isApprove
          ? `${B24Emoji.SUCCESS} Сообщение обработано: Сайт согласован`
          : `${B24Emoji.REFUSAL} Сообщение обработано: Сайт не согласован`) +
        `[/b][br][br]` +
        this.bitrixService.generateDealUrl(dealId);
  ...
  private async handleApprovePaymentAdvert(
    ...
    try {
      const { isBudget, dealId, userId, dialogId, yandex_direct_login } =
        fields;
      const [userName, action, price, contract, organization, , inn = ''] =
        message.split(' | ');
      const clearPrice = this.bitrixService.clearNumber(price);
      const clearInn = this.bitrixService.clearNumber(inn);
      const batchCommands: B24BatchCommands = {};
      ...
      // let generateInvoiceNumberMessage: string;
      //
      // try {
      //   if (yandex_direct_login) {
      //     // Генерируем счет
      //     const invoiceData =
      //       await this.yandexDirectService.generateInvoiceNumberByLogin({
      //         login: yandex_direct_login,
      //         with_file: true,
      //       });
      //
      //     // Если получилось достать счет: отправляем данные в wiki service
      //     if (invoiceData) {
      //       this.wikiService.sendReceivedPaymentData({
      //         amount: clearPrice,
      //         payment_type: 'rs',
      ...
      //         handler: this.handleApprovePayment.name,
      //         request: fields,
      //         error: `Не удалось получить счет по Login: ${yandex_direct_login}`,
      //       });
      //     }
      //   }
      //
      //   generateInvoiceNumberMessage = 'Авто генерация счета[br]';
      // } catch (error) {
      //   this.logger.error({
      //     handler: this.handleApprovePayment.name,
      //     message: 'Ошибка при генерации счета или отправке данных в Wiki',
      //     error: maybeCatchError(error),
      //   });
      //
      //   generateInvoiceNumberMessage =
      //     '[b]Произошла ошибка во время авто генерации счета[/b]. Необходимо вручную выставить счет';
      // }
      //
      // batchCommands['notify_about_auto_generate_number'] = {
      //   method: 'im.message.add',
      //   params: {
      //     DIALOG_ID: userId,
      //     MESSAGE:
      ...
      batchCommands['get_assigned_user'] = {
        method: 'user.get',
        params: {
          filter: {
      ...
      batchCommands['get_assigned_department'] = {
