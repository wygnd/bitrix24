...
export class BitrixDealsController {
  ...
  @ApiOperation({
    ...
    description:
      'Выполняет поиск по сделкам по всем стадиям, кроме <b>Заморозка, Сделка успешна, Сделка провалена</b> и отправляет сообщение в чат, который был указан при запросе',
  })
